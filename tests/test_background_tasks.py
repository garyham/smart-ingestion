...
class BackgroundTaskTests(unittest.TestCase):
    ...
    def test_upload_submits_embedding_task(self, ingest, _models, delay):
        ingest.return_value = {
            "status": "ok",
        ...
        delay.assert_called_once_with(
            ingest.return_value, "dense", "sparse", "cpu"
        )
        self.assertEqual(result["embedding_task_run_id"], "embedding-run")

    @patch("background_tasks.embed_document.delay")
    @patch("background_tasks.ingest_upload_flow")
    def test_upload_skips_embedding_for_duckdb(self, ingest, delay):
        ingest.return_value = {"status": "ok", "artifact_type": "duckdb"}
    ...
    def test_ingest_flow_downloads_and_publishes(
        self, make_s3_client, publish_bundle, route_document
    ):
        ...
        publish_bundle.return_value = {"event": "ingestion.completed"}

        result = ingest_upload_flow.fn(EVENT)

        route_document.assert_called_once()
        publish_bundle.assert_called_once()
        self.assertEqual(result, publish_bundle.return_value)

    @patch("background_tasks.embed_ingestion")
    def test_embedding_task_runs_flow(self, embed_ingestion):
        completion = {"status": "ok", "artifact_type": "chunks"}

        embed_document.fn(completion, "dense", "sparse", "cpu")

        embed_ingestion.assert_called_once_with(
            completion, "dense", "sparse", "cpu"
        )
